...
import com.jieli.stream.player.data.beans.SDFileInfo;
...
public class BrowseFileDialog extends BaseDialogFragment implements IConstant, View.OnClickListener, AdapterView.OnItemClickListener,
    ...
    private void viewFiles(String filePath) {
        ArrayList<SDFileInfo> tmp = AppUtil.getFiles(filePath);
        if (tmp != null) {
            // 清空数据
            fileInfoList.clear();
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()){
            case R.id.create_dir_btn:
                ...
                inputContentDialog.show(getActivity().getFragmentManager(), "input_content_dialog");
                break;
            ...
            case R.id.dialog_file_confirm_btn:
                if(ROOT_PATH.equals(currentPath)){
                    ...
                    if(AppUtil.checkIsEmptyFolder(currentPath)){ // ok
                        String oldPath = ROOT_PATH + File.separator + MainApplication.getApplication().getAppName();
    ...
    private void controlDialog(int op){
        ...
        if(movingDialog == null){
            movingDialog = new NotifyDialog(true, R.string.moving_dir_tip);
        }
        switch (op){
            case ARGS_SHOW_DIALOG:
                if(!movingDialog.isShowing() && !getActivity().isFinishing()){
                    movingDialog.show(getActivity().getFragmentManager(), "moving_dialog");
                }
                break;
    ...
    private void controlDeleteDirDialog(String path, int op){
        ...
        if(deleteDirDialog == null){
            deleteDirDialog = new NotifyDialog(getString(R.string.dialog_tip), deleteMsg, R.string.cancel, R.string.confirm,
        ...
        switch (op){
            case ARGS_SHOW_DIALOG:
                if(!deleteDirDialog.isShowing() && !getActivity().isFinishing()){
                    deleteDirDialog.show(getActivity().getFragmentManager(), "delete_dir_dialog");
                }
                break;
            case ARGS_DISMISS_DIALOG:
                if(deleteDirDialog.isShowing()){
